...
-- Drop the existing foreign key constraint in CancellationDetails
ALTER TABLE CancellationDetails DROP CONSTRAINT FK_Booking_CancellationDetails;

-- Recreate the foreign key constraint with ON DELETE CASCADE
ALTER TABLE CancellationDetails
ADD CONSTRAINT FK_Booking_CancellationDetails FOREIGN KEY (BookingId) REFERENCES Booking (Booking_Id) ON DELETE CASCADE;

ALTER TABLE Booking ALTER COLUMN SeatsBooked INT NULL;
...
DELETE FROM Booking WHERE Booking_Id = 21;
